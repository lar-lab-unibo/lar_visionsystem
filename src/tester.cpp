...
tf::StampedTransform t_target;
bool t_target_catch = false;
tf::StampedTransform t_cam_marker;
tf::StampedTransform tf_temp;
tf::Transform tf_updated;
...
float joystick_bistate_angle = 0.0f;
bool bistate1 = true;
int bistate_counter = 0;
...
bool joystic_first_callback = false;
void joy_cb(const sensor_msgs::Joy& msg){
        joystic_first_callback = true;
...
main (int argc, char** argv)
{
        ...
        //Initialize Joystick
        {
                joystick_speed_max.x = joystick_speed_max.y = joystick_speed_max.z =  1.0f;
                ...
                joystick_set_point.position.x = 0.0f;
        ...
        while( ros::ok() && GLOBAL_NODE_ACTIVE_STATUS ) {
                ...
                if(current_mode==JOYSTICK_CONTROL) {
                        joystick_set_point.position.x +=  joystick_speed.x/300.f;
                        joystick_set_point.position.y +=  joystick_speed.y/300.f;
                        joystick_set_point.position.z +=  joystick_speed.z/50.f;


                        if(bistate1==0 && current_joystick_message.buttons[4] ) {
                                joystick_bistate_angle += M_PI/2.0f;
                                bistate_counter++;
                                t_target_catch = false;
                        }
                        if(joystic_first_callback) {
                                bistate1 = current_joystick_message.buttons[4];
                        }
                        ...
                        std:cout << "Z: "<<joystick_set_point.position.z<<std::endl;
                        try{

                                listener->lookupTransform("camera_rgb_frame", "lar_marker_800", ros::Time(0), tf_temp);

                                if(!t_target_catch) {
                                        tf_updated = tf_temp;

                                        tf::Transform tf_camera;
                                        MathUtils::poseToTF(comau_current_state.pose,tf_camera,false,1000.0f);
                                        tf_updated = tf_camera * tf_updated ;
                                ...
                                MathUtils::transformToSphereApproach(tf_updated,approach,
                                                                     joystick_set_point.position.z/1000.0f+0.45f,
                                                                     M_PI/1.0f-joystick_set_point.position.x,
                                                                     joystick_set_point.position.y,
                                                                     joystick_bistate_angle -M_PI/2.0f
                                                                     );

                                tf_broadcaster->sendTransform(tf::StampedTransform(tf_updated,  ros::Time(0), "base", "table"));
                                tf_broadcaster->sendTransform(tf::StampedTransform(approach,  ros::Time(0), "base", "table_approach"));
                                current_position = approach;
                        }
                        catch (tf::TransformException ex) {
                                ROS_ERROR("%s",ex.what());
                        }
